...
export function Login() {
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 pt-6">
      <h1 className="text-h5 font-medium">{strings.screens.login}</h1>
      <p className="text-body-sm text-[var(--fg-muted)]">{strings.login.why}</p>

      {onPhoneStep ? (
        <form className="flex flex-col gap-3" onSubmit={submitPhone}>
          <label htmlFor="login-phone" className="flex flex-col gap-2 text-body-sm">
          ...
          {state.name === 'enterPhone' && state.problem !== null ? (
            <p role="alert" data-testid="login-error" className="text-body-sm text-[var(--danger)]">
              {state.problem === 'phoneInvalid' ? strings.login.phoneInvalid : strings.login.failed}
            </p>
          ) : null}
          <Button
            type="submit"
      ...
      {onCodeStep ? (
        <form className="flex flex-col gap-3" onSubmit={submitCode}>
          ...
          <label htmlFor="login-code" className="flex flex-col gap-2 text-body-sm">
