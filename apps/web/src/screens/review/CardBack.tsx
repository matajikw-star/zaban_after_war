...
function Sentence({ parts }: { readonly parts: readonly SentencePart[] }) {
  return (
    <p dir="ltr" lang="en" className="text-body leading-7">
...
export function CardBack({ card }: CardBackProps) {
  ...
  return (
    <div className="flex flex-1 flex-col gap-5 pt-2" data-testid="review-back">
      <div className="flex flex-col items-center gap-2 text-center">
        <span dir="ltr" lang="en" className="text-h4 font-medium tracking-tight break-words">
          {card.lemma}
        </span>
        <h2 className="text-h5 font-medium" data-testid="review-translations">
          {primary === null ? '—' : primary.translations.join(strings.format.listSeparator)}
        </h2>
        {badge === null ? null : <Chip tone="neutral">{examBadgeText(badge)}</Chip>}
      </div>

      {sentence === null ? null : (
        <div className="flex flex-col gap-2 rounded-[var(--radius-control)] bg-[var(--bg-muted)] p-4">
                ...
                <ul className="flex flex-col gap-1">
                  {card.confusables.map((confusable) => (
                    <li key={confusable.word}>
                      <bdi dir="ltr" lang="en" className="font-medium">
                        {confusable.word}
                      </bdi>{' '}
                      — {confusable.note}
                    </li>
