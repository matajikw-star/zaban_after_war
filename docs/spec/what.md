...
Each screen graduates from a placeholder to `[live]` on its own ticket. A path that matches
nothing renders a Persian not-found screen inside the same layout, because the service worker
answers every navigation with `index.html` (§7.7).
