...
from common import (OCR_CACHE, PART_A, PART_B, PART_C, RAW, STATE, english_score,
    ...
    try:
        with fitz.open(RAW / name) as doc:
            ...
            for pno in range(2, min(n, PROBE_UNTIL) + 1):
                s = english_score(page_text(doc, pno, booklet))
                scores[pno] = s
                if s["isEnglish"]:
                ...
                for pno in range(PROBE_UNTIL + 1, n + 1, DEEP_STEP):
                    ...
                    if s["isEnglish"]:
                        ...
                        for q in range(lo, hi + 1):
                            if q not in scores:
                                scores[q] = english_score(page_text(doc, q, booklet))
                        break
...
def main() -> int:
    ...
    todo = [b for b in booklets
            if b["year"] in want and (a.redo or b["bookletId"] not in routes)]
